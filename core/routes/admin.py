...
@admin_required
def add_member_to_companionship(companionship_id):
    ...
    return render_template('add_member_to_companionship.html', companionship=companionship, all_members=all_members)
...
def add_member():
    ...
    if request.method == 'POST':
        ...
        email = request.form.get('email', '')
        phone = request.form.get('phone', '')
        companionship_id = request.form.get('companionship_id')

        if not name:
            flash('Name is required.', 'error')
            return render_template('add_member.html', districts=districts)
        ...
        member = Member(
            ...
            match_email=email,
            match_phone=phone,
            companionship_id=int(companionship_id) if companionship_id else None,
            is_active=True
        )
        db.session.add(member)
        db.session.commit()
...
def toggle_member_active(member_id):
    """Toggle a member's is_active status"""
    member = Member.query.get_or_404(member_id)
    member.is_active = not member.is_active

    if not member.is_active:
        # When deactivating, optionally unassign from companionship
        # but keep the member record for history
        pass
...
def delete_member(member_id):
    """Delete a member permanently (including their bookings and notifications)"""
    member = Member.query.get_or_404(member_id)
    member_name = member.name

    # Count what will be deleted for the message
    booking_count = len(member.bookings)
    notification_count = len(member.notifications)

    # Delete bookings for this member
    Booking.query.filter_by(member_id=member_id).delete()

    # Delete notification logs for this member
    NotificationLog.query.filter_by(member_id=member_id).delete()
    ...
    # Build message
    msg = f'{member_name} has been deleted.'
    if booking_count > 0 or notification_count > 0:
        msg += f' Also removed: {booking_count} booking(s), {notification_count} notification(s).'
...
def import_progress(progress_id):
    """Return JSON progress data for import operations"""
    with progress_lock:
...
@admin_bp.route('/import_csv_confirm', methods=['GET'])
@admin_required
def import_csv_confirm():
    ...
    # Redirect to the new preview system
    return redirect(url_for('admin.import_preview', source='csv'))


@admin_bp.route('/import_preview')
@admin_required
def import_preview():
    """Show smart merge preview for import data"""
    from core.import_merge import build_import_preview

    source = request.args.get('source', 'csv')
    progress_id = request.args.get('progress_id')

    # Get import data based on source
    if source == 'csv':
        import_data = session.get('uploaded_districts')
        if not import_data:
            flash('No uploaded data found.')
            return redirect(url_for('admin.import_csv'))
    else:
        # For scraping, get from progress_store
        if not progress_id:
        ...
        with progress_lock:
            ...
            if not progress_data or progress_data['status'] != 'completed':
                ...
                return redirect(url_for('scraping.scrape_data'))
            import_data = progress_data.get('scraped_districts')
    ...
    # Build the preview
    preview = build_import_preview(import_data)

    # Store preview in progress_store for the execute route
    preview_id = str(uuid.uuid4())
    with progress_lock:
        progress_store[preview_id] = {
            'preview': preview,
            'source': source,
    ...
    execute_endpoint = 'admin.import_execute'
    cancel_url = url_for('admin.import_csv') if source == 'csv' else url_for('scraping.scrape_data')
...
@admin_bp.route('/import_execute', methods=['POST'])
@admin_required
def import_execute():
    """Execute the import with smart matching"""
    from core.import_merge import execute_import
    ...
    with progress_lock:
        preview_data = progress_store.get(preview_id)

    if not preview_data or 'preview' not in preview_data:
        flash('Preview data expired or not found. Please start over.')
        return redirect(url_for('admin.import_csv'))

    preview = preview_data['preview']
    source = preview_data.get('source', 'csv')

    # Check if user wants to clear all existing data first
    clear_existing = 'clear_existing' in request.form

    # Collect ambiguous decisions from form
    ambiguous_decisions = {}
    update_contact_decisions = {}
    for key, value in request.form.items():
        if key.startswith('ambiguous_') and not key.startswith('ambiguous_') == False:
            # Check it's just "ambiguous_X" not "update_contact_..."
            parts = key.split('_')
            if len(parts) == 2 and parts[0] == 'ambiguous':
                idx = parts[1]
                ambiguous_decisions[idx] = value
        elif key.startswith('update_contact_'):
            # Format: update_contact_<idx>_<member_id>
            parts = key.replace('update_contact_', '').split('_')
            if len(parts) == 2:
                idx, member_id = parts
                update_contact_decisions[idx] = member_id
    ...
    try:
        ...
        if clear_existing:
            ...
            db.session.commit()
            flash('Cleared all existing data.', 'info')

        # Execute the import
        result = execute_import(preview, ambiguous_decisions, update_contact_decisions, skip_delete=clear_existing)

        # Clean up
        with progress_lock:
            if preview_id in progress_store:
                del progress_store[preview_id]

        # Clear session data for CSV imports
        if source == 'csv':
            session.pop('uploaded_districts', None)
        ...
        messages = []
        if result['updated'] > 0:
            messages.append(f"{result['updated']} members updated")
        if result['created'] > 0:
            messages.append(f"{result['created']} new members created")
        if result['deactivated'] > 0:
            messages.append(f"{result['deactivated']} members deactivated (preserved with history)")
        if result['deleted'] > 0:
            messages.append(f"{result['deleted']} members deleted")

        flash(f"Import completed: {', '.join(messages)}", 'success')
        return redirect(url_for('admin.manage_members'))

    except Exception as e:
        db.session.rollback()
        flash(f'Import failed: {str(e)}', 'error')
        return redirect(url_for('admin.import_preview', source=source))
